...
app.get("/api/admin/students",auth("admin"),(req,res)=>{
  res.json(db.prepare("SELECT id,username,name FROM users WHERE role='student' ORDER BY id DESC").all());
});
app.post("/api/admin/students",auth("admin"),(req,res)=>{
  ...
  try {
    ...
    const r=db.prepare("INSERT INTO users(username,password_hash,role,name) VALUES(?,?,?,?)")
      .run(username,hash,"student",name);
    res.json({id:r.lastInsertRowid});
  } catch(e){res.status(400).json({error:"Username already exists"});}
});

app.get("/api/admin/exams",auth("admin"),(req,res)=>{
  res.json(db.prepare(`
    SELECT e.*, COUNT(q.id) question_count
    FROM exams e LEFT JOIN questions q ON q.exam_id=e.id
    GROUP BY e.id ORDER BY e.id DESC`).all());
});
app.post("/api/admin/exams",auth("admin"),(req,res)=>{
  ...
  const r=db.prepare("INSERT INTO exams(title,duration_minutes) VALUES(?,?)").run(title,Number(duration_minutes));
  res.json({id:r.lastInsertRowid});
});

app.post("/api/admin/questions/upload",auth("admin"),upload.single("file"),(req,res)=>{
  try {
    ...
    const tx=db.transaction(rows=>{
      ...
      for(const r of rows){
        ...
        insert.run(examId,r.question,r.option_a,r.option_b,r.option_c,r.option_d,answer,Number(r.marks||1)); n++;
      } return n;
    });
    const count=tx(rows);
...
app.get("/api/admin/results",auth("admin"),(req,res)=>{
  res.json(db.prepare(`
    ...
    WHERE a.submitted_at IS NOT NULL ORDER BY a.submitted_at DESC`).all());
});

app.get("/api/student/exams",auth("student"),(req,res)=>{
  res.json(db.prepare(`
    ...
    GROUP BY e.id ORDER BY e.id DESC`).all(req.session.user.id));
});

app.post("/api/student/exams/:id/start",auth("student"),(req,res)=>{
  const exam=db.prepare("SELECT * FROM exams WHERE id=?").get(req.params.id);
  if(!exam) return res.status(404).json({error:"Exam not found"});
  let attempt=db.prepare("SELECT * FROM attempts WHERE exam_id=? AND student_id=?").get(exam.id,req.session.user.id);
  if(attempt && attempt.submitted_at) return res.status(400).json({error:"Exam already submitted"});
  if(!attempt){
    const r=db.prepare("INSERT INTO attempts(exam_id,student_id,started_at) VALUES(?,?,?)")
      .run(exam.id,req.session.user.id,Date.now());
    attempt=db.prepare("SELECT * FROM attempts WHERE id=?").get(r.lastInsertRowid);
  }
  const questions=db.prepare("SELECT id,question,option_a,option_b,option_c,option_d,marks FROM questions WHERE exam_id=? ORDER BY id").all(exam.id);
  const existing=db.prepare("SELECT question_id,selected FROM answers WHERE attempt_id=?").all(attempt.id);
  res.json({exam,attempt,questions,answers:existing});
});

app.post("/api/student/attempts/:id/answer",auth("student"),(req,res)=>{
  const a=db.prepare("SELECT * FROM attempts WHERE id=? AND student_id=?").get(req.params.id,req.session.user.id);
  ...
  const q=db.prepare("SELECT id FROM questions WHERE id=? AND exam_id=?").get(req.body.question_id,a.exam_id);
  if(!q) return res.status(400).json({error:"Invalid question"});
  db.prepare(`INSERT INTO answers(attempt_id,question_id,selected) VALUES(?,?,?)
    ON CONFLICT(attempt_id,question_id) DO UPDATE SET selected=excluded.selected`)
    .run(a.id,q.id,req.body.selected||null);
  res.json({ok:true});
});

app.post("/api/student/attempts/:id/submit",auth("student"),(req,res)=>{
  const a=db.prepare("SELECT * FROM attempts WHERE id=? AND student_id=?").get(req.params.id,req.session.user.id);
  ...
  const score=db.prepare(`
    ...
    WHERE q.exam_id=?`).get(a.id,a.exam_id).score;
  db.prepare("UPDATE attempts SET submitted_at=?,score=? WHERE id=?").run(Date.now(),score,a.id);
  res.json({score});
});
...
app.get("/api/student/results",auth("student"),(req,res)=>{
  res.json(db.prepare(`
    ...
    WHERE a.student_id=? AND a.submitted_at IS NOT NULL GROUP BY a.id`).all(req.session.user.id));
});
